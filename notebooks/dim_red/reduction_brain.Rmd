...
heatmap.with.labels(brain.md.cor,min=-1,max=1,'correlation')
...
heatmap.with.labels(brain.fa.cor,min=-1,max=1,'correlation')
...
#Varimax
ncomp = 3
raw.loadings      = brain.pca$rotation[,1:ncomp] %*% diag(brain.pca$sdev, ncomp, ncomp)
rotated.loadings  = varimax(raw.loadings)$loadings
inv.loadings      = t(pracma::pinv(rotated.loadings))
brain.varimax.scores = scale(merged.brain.new) %*% inv.loadings
```
...
```{r}
write.csv(rotated.loadings,"tables/dim_red/brain.varimax.loadings.csv")
```
...
brain.latents$brain_latent_pc1 = brain.varimax.scores[,1]
brain.latents$brain_latent_pc2 = brain.varimax.scores[,2]
brain.latents$brain_latent_pc3 = brain.varimax.scores[,3]
```

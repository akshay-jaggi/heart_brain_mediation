...
```{r}
gMD.final = 
  rbind(dplyr::select(gMD.table[gMD.table$op == '=~',],c('rhs','est.std')),
        gMD.metrics.frame)
write.csv(gMD.final,"tables/final/gMD.cfa.csv")
```
...
```{r}
gFA.final = 
  rbind(dplyr::select(gFA.table[gFA.table$op == '=~',],c('rhs','est.std')),
        gFA.metrics.frame)
write.csv(gFA.final,"tables/final/gFA.cfa.csv")
```

...
```{r}
lin.to.save.residualized.final = dplyr::select(lin.to.save, -c('z','se'))
...
```{r}
ggplot(gvrf.heart.brain, aes(x = class, y = r, fill = class)) + 
...
```{r}
ggplot(g.heart.brain, aes(x = class, y = r, fill=class)) + 
  geom_boxplot(outlier.shape = NA) + 

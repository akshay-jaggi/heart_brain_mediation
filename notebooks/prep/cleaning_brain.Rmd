...
# Add in white matter hyperintensities
```{r}
brain.wmh = read.csv("data/wmh.csv")
brain.wmh = data.frame('f.eid' = brain.wmh$f.eid, 'wmh' = brain.wmh$f.25781.2.0)
```

```{r}
brain.wmh.final = brain.wmh[complete.cases(brain.wmh),]
```
...
```{r}
brain.mri.traditional= list(brain.volumes, brain.wmh.final, brain.md, brain.fa) %>% 
...
```{r}
brain.mri.new = list(brain.all.volumes, brain.wmh.final, brain.weighted.means) %>% 
  purrr::reduce(inner_join,by="f.eid")
```
